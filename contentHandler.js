...
eventsRef.on('value', function(snapshot){
  ...
  //if(localStorage.getItem("isAdmin")=="true"){
    if(snapshot.exists()){
        ...
        if(localStorage.getItem("isAdmin")=="true"){ //non-admin user
          //  if(isUserParticipatingInCourse(val)){ //check if user participating
              content +='<th>Action</th>';    //added
            //}
          }        
        ...
        snapshot.forEach(function(data){
          ...
          //  if(isUserParticipatingInCourse(val)){ //check if user participating
           ...
           content +=  '<td><button type="button" class="btn btn-success">Start Event</button></td>';       
...
function loadCourses(){
	database.ref('/courses').on('value', function(snap){
		...
		var x;
		for(x in res){
      display += '<span style="margin-left:2em">'
			display += '<input id="checkbox" type="checkbox">' + '             ' + res[x].name + '</input></span></br>';
		}
